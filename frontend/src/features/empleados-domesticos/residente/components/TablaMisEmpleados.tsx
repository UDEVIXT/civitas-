...
export function TablaMisEmpleados({ items, onEditClick }: MisEmpleadosTableProps) {
  return (
              ...
              <TableRow key={empleado.id}>
                <TableCell>
                  <div className="flex items-center gap-3">
                    <div className={cn("flex size-10 items-center justify-center rounded-full text-sm font-semibold", avatarPalette[index % avatarPalette.length])}>
                      {getInitials(empleado.nombre)}
                    </div>
                    <div>
                      <p className="text-sm font-medium text-foreground">{empleado.nombre}</p>
                      <p className="text-xs text-muted-foreground">{empleado.telefono}</p>
                    </div>
                  </div>
                </TableCell>
                <TableCell>
                  <Badge className={statusStyles[empleado.estado] || statusStyles.Inactivo}>
                    {empleado.estado}
                  </Badge>
                </TableCell>
